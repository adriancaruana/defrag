...
def binomial_tree(n: int):
    """Returns a binomial tree graph."""
    G = nx.generators.binomial_tree(n, create_using=nx.DiGraph())
    G = G.reverse()
    return G

def gnp_random_graph(n: int, p: float, seed: int = None, directed: bool = True):
...
@py_random_state(4)
def directed_extended_barabasi_albert_graph(n: int, m: int, p: float, q: float, seed: int = None):
